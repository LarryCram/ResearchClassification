...
ANZSRC (FOR/SEO), OpenAlex, ASJC and the SDGs}
\author{}
\date{}
...
% documentation; OpenAlex topic classification documentation; Elsevier ASJC
% documentation; UN SDG indicator framework) at the points marked [CITE] below, using
% this paper's own bibliography style.
...
(1998/2008/2020), OpenAlex (domain/field/subfield/topic), Elsevier's All Science Journal
Classification (ASJC), the United Nations Sustainable Development Goals (SDGs), and a
user-provided five-area aggregate over the FOR2020 divisions (\texttt{FOR2020\_AREA5}).
The repository exposes this
...
keywords and summary). It is tracked directly in the repository's data directory
precisely because the keyword
...
\subsection{A user-provided FOR2020 five-area aggregate}

A coarse, top-level grouping above the 23 FOR2020 divisions is provided directly by the
user as a five-category aggregate (Life and Earth Science; Biomedical and Health Science;
Physical Science and Engineering; Social Science and Humanities; Mathematics, Computing
and Information Science), with Indigenous Studies and the Multidisciplinary catch-all
category handled separately. \texttt{resolve()} exposes this as \texttt{FOR2020\_AREA5}, a
direct division-code lookup (\texttt{match\_method = "user\_provided"}, confidence 1.0
throughout) reachable from any FOR vintage or from OpenAlex via the FOR2020 hub.

An earlier version of this repository instead derived a comparable coarse grouping from
the CWTS Leiden Ranking's five ``main fields,'' joining Leiden micro-clusters to OpenAlex
topics via an exact match on a shared Wikipedia URL and then taking a majority vote over
FOR2020 divisions/groups through that join. That mechanism has since been removed
entirely: on inspection, the Leiden linkage fed nothing else in the pipeline (it was not,
despite an earlier assumption, an input to the OpenAlex$\leftrightarrow$FOR2020
correspondence described in Section~\ref{sec:extension}, which is built purely from
lexical matching between OpenAlex and FOR2020 labels), and it added an indirect,
two-hop derivation (FOR2020 $\rightarrow$ OpenAlex $\rightarrow$ Leiden) where the direct,
user-provided fact above is simpler, more transparent, and does not depend on any
CWTS-sourced data at all.
...
``Pacific Peoples archaeology''). With no direct OpenAlex or ASJC counterpart, and an
aggregate vocabulary large enough to distort any bag-based matching, it is
...
FOR2020 $\rightarrow$ OpenAlex field & 2{,}203 & 2{,}203 (100\%) & 0 \\
FOR1998 $\rightarrow$ OpenAlex field & 1{,}060 & 1{,}060 (100\%) & 0 \\
FOR2008 $\rightarrow$ OpenAlex field & 1{,}420 & 1{,}420 (100\%) & 0 \\
FOR2020 $\rightarrow$ FOR2020\_AREA5 & 2{,}203 & 2{,}203 (100\%) & 0 \\
\bottomrule
\end{tabular}
...
26 fields and 4 domains), 361 ASJC codes, 22 SDG goals/pillars, and a 23-division
\texttt{FOR2020\_AREA5} aggregate, cross-linked by 9 bridge tables (including an
empirical FOR2008$\leftrightarrow$FOR2020 correspondence derived independently from ARC
ERA journal classifications, cross-validated against --- and agreeing exactly with, at
division level --- the official ABS correspondence). The automated test suite comprises
30 tests, exercised against the pre-built database on every change, and passes in full
as of this document.
