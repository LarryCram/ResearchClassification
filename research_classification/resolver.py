...
class Resolver:
    ...
    def _bridge_lookup(self, value: str, system: System, by: str) -> list[CanonicalResult]:
        """Search every bridge table for a matching source_code (by='code') or
        case-insensitive source_label (by='label'), is_primary row first."""
        ...
        for table in _BRIDGE_TABLES:
            ...
            for source_system, code, label, level, is_primary, match_method, confidence in self._con.execute(
                query, [system, value]
            ).fetchall():
    ...
    def resolve(self, value: str, system: System) -> CanonicalResult:
        if system not in ("FOR", "SEO", "OAX"):
            raise ValueError(f"system must be one of FOR/SEO/OAX, got {system!r}")

        hit = self._canonical_identity(value, system)
        if hit:
            return hit

        bridge_hits = self._bridge_lookup(value, system, by="code")
        if bridge_hits:
        ...
        hit = self._canonical_label_match(value, system)
        if hit:
            return hit

        bridge_hits = self._bridge_lookup(value, system, by="label")
        if bridge_hits:
            primary, *rest = bridge_hits
            return CanonicalResult(
                primary.input_value, primary.system, primary.canonical_code, primary.canonical_label,
    ...
    def resolve_many(self, values: list[str], system: System) -> list[CanonicalResult]:
        return [self.resolve(v, system) for v in values]

    def resolve_forward(
        self, value: str, system: Literal["FOR", "SEO"], targets: tuple[str, ...] = ("FOR2020", "OAX", "Leiden")
        ...
        """
        base = self.resolve(value, system)
